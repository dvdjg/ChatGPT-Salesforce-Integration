...
generatePackageXml () {
    echo
    if [ -z "$1" ]; then
        read -e -p "$(echo -e "Generar ${Blue}package.xml${Clear} con el diff desde la rama acual ${Purple}${CURRENT_BRANCH}${Clear} hacia (dev1, dev2, int, uat, main, master...): ")" -i "$branchName" branchName
    else
        branchName="$1"
    fi
    local sino
    ...
    if echo "$SGD_OUT" | grep ": true"; then
        if [ -s "$DELTA_PACKAGE" ] && [ -s "$DELTA_DESTRUCTIVEPACKAGE" ]; then
            ...
            if grep -q "<members>" "$DELTA_DESTRUCTIVEPACKAGE"; then
                ...
                echo -e "Destructive Package generado en ${Purple}$DEPLOYMENT_DIR/${POSTDESTRUCTIVEPACKAGE_NAME}${Clear}. Se ejecutará en un proces posterior al deploy ordinario."
...
copyPackageFiles () {
    ...
    if [ -z "$packagePath" ]; then
        read -e -p "$(echo -e "Preparando los archivos de despliegue desde la rama actual ${Purple}${CURRENT_BRANCH}${Clear} según el ${Blue}package.xml${Clear}: ")" -i $DEPLOYMENT_PACKAGE packagePath
    fi
...
selectPackagePath () {
    ...
    read -e -p "$(echo -e "Indique la ruta del ${Blue}package.xml${Clear}. Deje ${Yellow}vacío${Clear} para usar toda la metadata de la rama, ignorando ${Blue}package.xml${Clear}: ")" -i "${DEPLOYMENT_PACKAGE}" packagePath
...
retrieveSourceMetadata () {
    ...
    if [ -z "$orgName" ]; then
        ...
        read -e -p "$(echo -e "Indique el alias correspondiente con la org de origen ${Green}($(echoAliases))${Clear}: ")" -i "$currentOrgRetrieveMetadata" orgName
...
deployMetadata () {
    ...
    if [ "$type" == "source" ] || [ -d "$CONVERTED_MAPI_DIR" ] || [ -s "$ARTIFACT_PATH" ]; then
        if [ -z "$orgName" ]; then
            ...
            read -e -p "$(echo -e "Indique el alias correspondiente con la org de destino (${Green}$(echoAliases)${Clear}): ")" -i "$currentOrgDeployMetadata" orgName
        ...
        mkdir -p "$DEPLOYMENT_DIR/"
        [ -z "$validarDesplegar" ] && read -e -p "$(echo -e "Desea validar o desplegar [v/d]: ")" -i "v" validarDesplegar
        ...
        if [ "$runLocalTests" == "s" ]; then 
            ...
            read -e -p "$(echo -e "Indique los test de la entrega. Separar por comas y sin espacios (${Blue}Enter${Clear} si no quiere ejecutar test): ")" -i "$testList" testList
            if [ -z "$testList" ]; then
        ...
        if [ "$type" == "source" ]; then
            ...
            if [ -z "$packagePath" ]; then
                ...
                if [ -s "${PACKAGEDIR}/$filename" ]; then
                    command="sfdx force:source:deploy $depTest $checkOnly $uUserName -x ${PACKAGEDIR}/$filename -w 20 --verbose"
...
branchPromotion () {
    ...
    if [ -z "$branchName" ]; then
        read -e -p "$(echo -e "Seleccione la rama de destino del ${Blackb}Pull-Request${Clear} a la que añadirá el contenido de la rama actual ${Purple}${CURRENT_BRANCH}${Clear} (dev1, dev2, dev3, int, uat, master...): ")" -i "int" branchName
    fi
...
deleteScratchOrg () {
    ...
    local devHubAlias="$2"
    if [ -z "$orgAlias" ]; then
        read -e -p "$(echo -e "Indique el alias correspondiente a la org de destino (${Green}$(echoAliases)${Clear}): ")" -i "" orgAlias
    fi
    if [ -z "$devHubAlias" ]; then
        read -e -p "$(echo -e "Indique el alias correspondiente a la DevHub org: ")" -i "devHub" devHubAlias
    fi
    if [ ! -z "$orgAlias" ]; then
...
createScratchOrg () {
    ...
    read -e -p "$(echo -e "Indique el nombre de la Org anfitrión DevHub: ")" -i "devHub" devhubOrg
